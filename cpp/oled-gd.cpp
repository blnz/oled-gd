...
#define RETURN_OLED(IMG)                                               \
  if (!IMG) return Null();                                              \
  HandleScope scope;                                                    \
  Local<Value> _arg_ = External::New(IMG);                              \
  Persistent<Object> _oled_(Oled::constructor_template->GetFunction()->NewInstance(1, &_arg_)); \
  return scope.Close(_oled_);
...
public:
  static void Init(Handle<Object> target)
  {
    ...
    NODE_SET_METHOD(target, "oled", OledCreate);
...
protected:
  ...
  static Handle<Value> OledCreate (const Arguments& args)
  {
    
    oledPtr ol = oled_create();
  ...
  public:
    ...
    static void Init(v8::Handle<Object> target) {
      ...
      NODE_SET_PROTOTYPE_METHOD(t, "stringBIF", StringBIF);
  ...
  protected:
    ...
    static Handle<Value> StringBIF (const Arguments &args)
    {
      REQ_ARGS(5);
      REQ_INT_ARG(0, color);
      REQ_STR_ARG(1, font);
      ...
      if (!strcmp(*font, "small")) {
        fp = gdFontGetSmall();
      } else if (!strcmp(*font, "medium")) {
        fp = gdFontGetMediumBold();
      } else if (!strcmp(*font, "large")) {
        fp = gdFontGetLarge();
      } else if (!strcmp(*font, "giant")) {
        fp = gdFontGetGiant();
      } else if (!strcmp(*font, "tiny")) {
        fp = gdFontGetTiny();
      } else {
        fp = gdFontGetSmall();
      }
      ...
      gdImageString(*im, fp, x, y, (unsigned char*) *str, color);
  ...
  public:
    ...
    static void Init(v8::Handle<Object> target) {
      ...
      NODE_SET_PROTOTYPE_METHOD(t, "demo", Demo);
      
      NODE_SET_PROTOTYPE_METHOD(t, "display", Display);
      
      NODE_SET_PROTOTYPE_METHOD(t, "clearDisplay", ClearDisplay);
      
      NODE_SET_PROTOTYPE_METHOD(t, "blitString", BlitString);
      
      NODE_SET_PROTOTYPE_METHOD(t, "invert", InvertDisplay);

      NODE_SET_PROTOTYPE_METHOD(t, "showImage", ShowImage);
      
      t->InstanceTemplate()->SetInternalFieldCount(1);

      target->Set(String::NewSymbol("Oled"), t->GetFunction());
    }

    ///////////////

    //
    static Handle<Value> Demo(const Arguments& args)
    {
      v8::HandleScope scope;
      Oled *ol = ObjectWrap::Unwrap<Oled>(args.This());
      oled_demo();
      return scope.Close(String::New("Hi World"));
    }
    
    // 
    // copies pixels from gdImage to oled's bitmap
    static Handle<Value> ShowImage(const Arguments& args)
    {
      v8::HandleScope scope;
      Oled *ol = ObjectWrap::Unwrap<Oled>(args.This());
      ...
      int lcd_bytes = ((LCD_WIDTH / 8) * LCD_HEIGHT);
      oledPtr op = *ol;
      uint8_t *dest = op->display_bitmap;
      ...
      for (int row = 0; row < LCD_HEIGHT; ++row) {
        for (int col = 0; col < LCD_WIDTH; ++col) {
          ...
          int srcPixel = gdImageGetPixel(*src, col + x, row + y);
          //   std::cout << "pixel: " << col << ", " << row << " = " << srcPixel << "\n";

          if (srcPixel > 0) {

            int destByte = ((row * LCD_WIDTH) + col) / 8;
            int destBit = ((row * LCD_WIDTH) + col) % 8;
            int destBitMask = 128 >> destBit;
    ...
    //
    // reverses black and white in a given rectangle
    //
    static Handle<Value> InvertDisplay(const Arguments& args)
    {
      int xoff = 0;
      int yoff = 0;
      int width = 48;
      ...
      if (args[0]->IsInt32()) {
        xoff = args[0]->Int32Value();
      }
      if (args[1]->IsInt32()) {
        yoff = args[1]->Int32Value();
      }
      if (args[2]->IsInt32()) {
      ...
      oledInvertDisplay(*ol, xoff, yoff, width, height);
    ...
    // deprecated
    //
    static Handle<Value> BlitString(const Arguments& args)
    {
      HandleScope scope;
      Oled *ol = ObjectWrap::Unwrap<Oled>(args.This());
      ...
      if (args[0]->IsString() ) {
        ...
        if (args[1]->IsInt32()) {
          xoff = args[1]->Int32Value();
        }
        if (args[2]->IsInt32()) {
          yoff = args[2]->Int32Value();
        }
        oledBlitString(*ol, *cstr, xoff, yoff);
        
      }
      return scope.Close(String::New("BlitString: done."));
    }
    
    //
    // sends the display's bitmap to the oled screen
    static Handle<Value> Display(const Arguments& args)
    {
      HandleScope scope;
      Oled *ol = ObjectWrap::Unwrap<Oled>(args.This());
      oledRenderDisplay(*ol);
      return scope.Close(String::New("Display: done."));
    }
    
    //
    static Handle<Value> ClearDisplay(const Arguments& args)
    {
      HandleScope scope;
      Oled *ol = ObjectWrap::Unwrap<Oled>(args.This());
      oledClearDisplay(*ol);
    ...
    static Handle<Value> New(const Arguments& args) {
      HandleScope scope;
      REQ_EXT_ARG(0, oledp);
      (new Oled((oledPtr)oledp->Value()))->Wrap(args.This());
      return args.This();
    }
  protected:
    Oled(oledPtr oledp) : _oled(oledp) {}
    ~Oled() { if(_oled) {}  }

    oledPtr _oled;

    operator oledPtr () const { return _oled; }
...
Persistent<FunctionTemplate> Gd::Oled::constructor_template;
